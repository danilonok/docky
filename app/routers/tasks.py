...
async def add_task_add_document_to_index(current_user: Annotated[User, Depends(get_current_active_user)], document_id: int, chat_id: int, session: SessionDep):
    document = session.exec(select(Document).where(Document.id == document_id and Document.user_id == current_user.id)).first()
...
async def add_task_query(current_user: Annotated[User, Depends(get_current_active_user)], query:str,  session: SessionDep):
