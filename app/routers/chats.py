...
@router.delete("/chats/{chatId}/documents", tags=["chats"], response_model=DocumentRead)
async def delete_documents_in_chat(current_user: Annotated[UserRead, Depends(get_current_active_user)], chatId: int, session: SessionDep) -> Chat | None:
    chat = chat_service.delete_documents_in_chat(session=session, chatId=chatId)
    
    return chat

@router.delete("/chats", tags=["chats"], response_model=DocumentRead)
async def delete_chat(current_user: Annotated[UserRead, Depends(get_current_active_user)], chat_id: int, session: SessionDep) -> Chat | None:
